...
const getWorker = () => {
    if (!worker) {
        const workerBlob = new Blob([WORKER_FILE_CONTENT], {
...
const decodeBuffer = async (buffer) => {
    return new Promise((resolve, reject) => {
        ...
        const handleEvent = (event) => {
            if (event.data.id === id) {
                workerInstance.removeEventListener('message', handleEvent);
                if (event.data.error) {
        ...
        workerInstance.addEventListener('message', handleEvent);
    });
};
    ...
    return await new Promise((resolve, reject) =>
        canvas.toBlob(
            (blob) => {
                ...
                else reject(`Can't convert canvas to blob.`);
            },
            type,
            quality
        )
